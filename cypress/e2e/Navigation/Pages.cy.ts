...
describe('Navigation', () => {
  ...
  it('Redirections de la page Dashboard', () => {
    ...
    cy.visitDashboard();
    cy.get('[class*="LinkBox_dataExploBox"]').eq(2).click({force: true}); // data-cy="GridCard_Biospecimens"
    cy.get('[class*="Biospecimens_biospecimenTabWrapper"]').should('exist'); // data-cy="ProTable_Biospecimens"

    cy.visitDashboard();
    cy.get('[class*="LinkBox_dataExploBox"]').eq(3).click({force: true}); // data-cy="GridCard_DataFiles"
  ...
  it('Liens de la page Profile', () => {
    ...
    cy.get('[class*="ProfileSettings_profileSettingsHeader"] button').click({force: true}); // data-cy="ViewProfileButton"
    cy.get('[class*="Member_bannerActions"]').find('[href]').eq(0).click({force: true}); // data-cy="CommunityButton"
    cy.get('[class*="Community_title"]').should('exist'); // data-cy="Title_Community"
  });
