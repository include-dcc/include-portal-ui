...
describe('Page des études - Vérifier les informations affichées', () => {
  ...
  it('Tableau', () => {
    ...
    cy.get('tr[data-row-key="HTP"] [class="ant-table-cell"]').eq(0).find('[class*="ant-tag-volcano"]').contains('G').should('exist');
    ...
    cy.get('tr[data-row-key="HTP"] [class="ant-table-cell"]').eq(5).contains('phs002981').should('exist');
    cy.get('tr[data-row-key="HTP"] [class="ant-table-cell"]').eq(6).contains('1,225').should('exist');
    cy.get('tr[data-row-key="HTP"] [class="ant-table-cell"]').eq(7).contains('-').should('exist');
    cy.get('tr[data-row-key="HTP"] [class="ant-table-cell"]').eq(8).contains('58,049').should('exist');
    cy.get('tr[data-row-key="HTP"] [class="ant-table-cell"]').eq(9).contains('11,753').should('exist');
    cy.get('tr[data-row-key="HTP"] [class="ant-table-cell"]').eq(10).contains('-').should('exist');
    cy.get('tr[data-row-key="HTP"] [class="ant-table-cell"]').eq(11).find('[data-icon="check"]').should('exist');
    cy.get('tr[data-row-key="HTP"] [class="ant-table-cell"]').eq(12).contains('-').should('exist');
    cy.get('tr[data-row-key="HTP"] [class="ant-table-cell"]').eq(13).contains('-').should('exist');
    cy.get('tr[data-row-key="HTP"] [class="ant-table-cell"]').eq(14).contains('-').should('exist');
  ...
  it('Summary', () => {
    ...
    cy.get('tfoot [class*="summarySum"]').eq(0).contains('1,225').should('exist');
    cy.get('tfoot [class*="summaryTitle"]').eq(1).contains('Families').should('exist');
    cy.get('tfoot [class*="summarySum"]').eq(1).contains('0').should('exist');
    cy.get('tfoot [class*="summaryTitle"]').eq(2).contains('Biospecimens').should('exist');
    cy.get('tfoot [class*="summarySum"]').eq(2).contains('58,049').should('exist');
    cy.get('tfoot [class*="summaryTitle"]').eq(3).contains('Files').should('exist');
    cy.get('tfoot [class*="summarySum"]').eq(3).contains('11,753').should('exist');
  });
});
