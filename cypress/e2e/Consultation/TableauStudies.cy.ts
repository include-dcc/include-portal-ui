...
describe('Page des études - Vérifier les informations affichées', () => {
  ...
  it('Tableau', () => {
    ...
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(9).contains(/\d{1}/).should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(10).find('[data-icon="check"]').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(11).find('[data-icon="check"]').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(12).find('[data-icon="check"]').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(13).contains('-').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(14).find('[data-icon="check"]').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(15).contains('The Human Trisome Project (').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(16).contains('Pediatric').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(16).contains('Adult').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(17).contains('Investigator Assessment').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(17).contains('Medical Record').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(18).contains('Case-control').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(18).contains('longitudinal').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(19).contains('Joaquin M. Espinosa').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(20).contains('2016').should('exist');
    cy.get('tr[data-row-key="HTP"]').find('[class="ant-table-cell"]').eq(21).contains('-').should('exist');
  });
});
