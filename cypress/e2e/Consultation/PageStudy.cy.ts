...
describe('Page d\'une étude - Vérifier les informations affichées', () => {
  ...
  it('Panneau Summary', () => {
    ...
    cy.get('[id="summary"] [class="ant-descriptions-item-label"]').eq(4).contains('Participant Lifespan').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(4).find('[class*="ant-tag-cyan"]').contains('Pediatric').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(4).find('[class*="ant-tag-cyan"]').contains('Adult').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-label"]').eq(5).contains('Description').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(5).contains('The Human Trisome Project (HTP) is a large and comprehensive natural history study of Down syndrome involving collection of deep clinical data, multimodal phenotyping, a multi-dimensional biobank, generation of pan-omics datasets, and rapid release of data. The HTP has enabled many discoveries about the pathophysiology of Down syndrome, leading to new clinical trials testing therapies to improve diverse health outcomes in this population.').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-label"]').eq(6).contains('Research Domain').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(6).contains('All Co-occurring Conditions').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-label"]').eq(7).contains('Clinical Data Source Type').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(7).contains('Investigator Assessment, Medical Record, Participant or Caregiver Report').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-label"]').eq(8).contains('Date Collection Start (Year)').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(8).contains('2016').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-label"]').eq(9).contains('Date Collection End (Year)').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(9).contains('-').should('exist');
    ...
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(13).contains('https://pubmed.ncbi.nlm.nih.gov/37379383/').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(13).contains('https://pubmed.ncbi.nlm.nih.gov/37360690').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(13).contains('https://pubmed.ncbi.nlm.nih.gov/37277650').should('exist');
    ...
    cy.get('[id="summary"] [class="ant-descriptions-item-label"]').eq(16).contains('Virtual Biorepository Email').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(16).contains('dsresearch@cuanschutz.edu').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-label"]').eq(17).contains('Virtual Biorepository URL').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(17).contains('https://redcap.link/HTPVBRrequest').should('exist');
  });
...
describe('Page d\'une étude - Valider les liens disponibles', () => {
  ...
  it('Lien \'See more\' de Publication du panneau Summary', () => {
    ...
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(13).contains('https://pubmed.ncbi.nlm.nih.gov/36577365').should('exist');
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(13).contains('See less').clickAndWait({force: true});
    cy.get('[id="summary"] [class="ant-descriptions-item-content"]').eq(13).contains('https://pubmed.ncbi.nlm.nih.gov/36577365').should('not.exist');
  });
