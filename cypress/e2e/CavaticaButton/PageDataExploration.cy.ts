...
describe('Page Dashboard - Bouton Analyze in Cavatica (déconnecté)', () => {
  beforeEach(() => {
    ...
    cy.get('tr[data-row-key="HTP.ce770763-b904-4c66-8b08-c54dd4e7c7b5.rsem.isoforms.results.gz"]').find('[type="checkbox"]').check({force: true});
...
describe('Page Dashboard - Bouton Analyze in Cavatica (connecté)', () => {
  ...
  it('Vérifier les informations affichées - Modal', () => {
    ...
    cy.get('[class*="CavaticaAnalyzeModal_studiesList"]').contains('The Human Trisome Project').should('exist');
